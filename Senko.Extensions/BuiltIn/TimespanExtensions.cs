...
public static class TimespanExtensions {
    ...
    public static string AsLongFormat(this TimeSpan timeSpan) {
        ...
        if (timeSpan.Days != 0) strings.Add(timeSpan.Days + " " + "Day".Plural(timeSpan.Days));
        if (timeSpan.Hours != 0) strings.Add(timeSpan.Hours + " " + "Hour".Plural(timeSpan.Hours));
        if (timeSpan.Minutes != 0) strings.Add(timeSpan.Minutes + " " + "Minute".Plural(timeSpan.Minutes));
        if (timeSpan.Seconds != 0) strings.Add(timeSpan.Seconds + " " + "Second".Plural(timeSpan.Seconds));
        if (strings.Count == 1)
            return strings[0];
